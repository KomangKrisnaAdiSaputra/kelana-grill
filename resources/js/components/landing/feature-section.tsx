...
export default function FeatureSection({ theme }: Props) {
    ...
    return (
        <section id="features" className="py-20 md:py-5">
            <div className="mx-auto max-w-7xl px-4 md:px-6">
                <div className="max-w-2xl">
                    ...
                    <p
                        className={`mt-6 text-base leading-7 md:text-lg md:leading-8 ${
                ...
                <div className="mt-10 grid gap-4 sm:grid-cols-2 md:mt-16 md:gap-6 xl:grid-cols-4">
                    {featuresItems.map((item, index) => (
                        <div
                            key={index}
                            className={`group relative overflow-hidden rounded-[28px] border p-6 backdrop-blur-xl transition duration-500 hover:-translate-y-2 hover:shadow-[0_20px_80px_rgba(255,140,0,0.15)] md:rounded-[32px] md:p-8 ${
                                theme === 'dark'
                                    ? 'border-white/10 bg-white/[0.03] hover:border-orange-400/30'
                                    : 'border-orange-100 bg-white/70 hover:border-orange-300'
                            } `}
                        >
                            <div className="absolute top-0 right-0 h-32 w-32 bg-orange-500/10 blur-3xl transition group-hover:bg-orange-500/20" />
